...
class WebSocket(object):
    ...
    async def create(request):
        ...
        rider_id = request.rel_url_query.get("rider_id")


        if app.connections.get(rider_id):
            raise HTTPBadRequest
        else:
            app.connections[rider_id] = ws
            app.coordinates[rider_id] = [0.0, 0.0]
        try:
            while True:
                msg = await ws.recv()
                if msg.type == WSMsgType.TEXT:
                    if msg.data = "close":
                        await _disconnect_user(rider_id, app)
                    rider_coordinates = msg.data
                    app.coordinates[rider_id] = rider_coordinates
                    await self.broadcast_message(rider_coordinates)    
                    # else:
            ...
            await self._disconnect_user(rider_id, app)
        finally:
            await self._disconnect_user(rider_id, app)

    async def brodcast_message(rider_coordinates, app):
