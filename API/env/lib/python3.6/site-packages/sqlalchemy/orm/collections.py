...
``update``, ``__setslice__``, etc.) into the series of atomic mutation event
...
duplicate event, which should be avoided.
...
Tight control over bulk operation and the firing of event is also possible by
implementing the instrumentation internally in your methods.  The basic
instrumentation package works under the general assumption that collection
mutation will not raise unusual exceptions.  If you want to closely
orchestrate append and remove event with exception management, internal
instrumentation may be the answer.  Within your method,
``collection_adapter(self)`` will retrieve an object that you can use for
explicit control over triggering append and remove event.
...
class collection(object):
    ...
    def appender(fn):
        ...
        promulgation to collection event.
    ...
    def internally_instrumented(fn):
        ...
        decoration from wrapping your implementation::
            ...
            # SQLAlchemy event and append().  your implementation will
...
class CollectionAdapter(object):
    ...
    to the underlying Python collection, and emits add/remove event for
    ...
    def append_with_event(self, item, initiator=None):
        """Add an entity to the collection, firing mutation event."""

        self._data()._sa_appender(item, _sa_initiator=initiator)

    def append_without_event(self, item):
        """Add or restore an entity to the collection, firing no event."""
    ...
    def remove_with_event(self, item, initiator=None):
        """Remove an entity from the collection, firing mutation event."""
        self._data()._sa_remover(item, _sa_initiator=initiator)

    def remove_without_event(self, item):
        """Remove an entity from the collection, firing no event."""
    ...
    def clear_without_event(self):
        """Empty the collection, firing no event."""
...
def bulk_replace(values, existing_adapter, new_adapter, initiator=None):
    """Load a new collection, firing event based on prior like membership.

    Appends instances in ``values`` onto the ``new_adapter``. Events will be
    fired for any instance not present in the ``existing_adapter``.  Any
    instances in ``existing_adapter`` not present in ``values`` will have
    remove event fired upon them.
...
def _list_decorators():
    ...
    # decorator is still possible, so event on *= can be had if they're
