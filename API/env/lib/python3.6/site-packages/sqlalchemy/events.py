...
class DDLEvents(event.Events):
    ...
    :class:`_schema.MetaData` and :class:`_schema.Table` support event
    specifically regarding when CREATE and DROP
    DDL is emitted to the database.

    Attachment event are also provided to customize
    ...
    DDL event integrate closely with the
    ...
    For all :class:`.DDLEvent` event, the ``propagate=True`` keyword argument
    ...
    def before_create(self, target, connection, **kw):
        ...
        :param \**kw: additional keyword arguments relevant
         ...
         elements used by internal event.
...
class PoolEvents(event.Events):
    """Available event for :class:`_pool.Pool`.
    ...
    def close(self, dbapi_connection, connection_record):
        ...
        associated with the pool. To intercept close event for detached
...
class ConnectionEvents(event.Events):
    """Available event for :class:`.Connectable`, which includes
    ...
    event can also be established with the ``retval=True`` flag, which
    ...
    :param retval=False: Applies to the :meth:`.before_execute` and
      :meth:`.before_cursor_execute` event only.  When True, the
    ...
    def before_execute(self, conn, clauseelement, multiparams, params):
        """Intercept high level execute() event, receiving uncompiled
    ...
    def after_execute(self, conn, clauseelement, multiparams, params, result):
        """Intercept high level execute() event after execute.
    ...
    def before_cursor_execute(
        ...
        """Intercept low-level cursor execute() event before execution,
    ...
    def after_cursor_execute(
        ...
        """Intercept low-level cursor execute() event after execution.
    ...
    def rollback(self, conn):
        """Intercept rollback() event, as initiated by a
        :class:`.Transaction`.
    ...
    def commit(self, conn):
        """Intercept commit() event, as initiated by a
        :class:`.Transaction`.
    ...
    def rollback_savepoint(self, conn, name, context):
        """Intercept rollback_savepoint() event.
    ...
    def release_savepoint(self, conn, name, context):
        """Intercept release_savepoint() event.
    ...
    def begin_twophase(self, conn, xid):
        """Intercept begin_twophase() event.
    ...
    def prepare_twophase(self, conn, xid):
        """Intercept prepare_twophase() event.
    ...
    def rollback_twophase(self, conn, xid, is_prepared):
        """Intercept rollback_twophase() event.
    ...
    def commit_twophase(self, conn, xid, is_prepared):
        """Intercept commit_twophase() event.
...
class DialectEvents(event.Events):
    ...
    These event allow direct instrumentation and replacement
    ...
    .. note::
        ...
        an existing dialect.  For general-use statement-interception event,
    ...
    def do_connect(self, dialect, conn_rec, cargs, cparams):
        ...
        Return a DBAPI connection to halt further event from invoking;
    ...
    def do_executemany(self, cursor, statement, parameters, context):
        ...
        Return the value True to halt further event from invoking,
