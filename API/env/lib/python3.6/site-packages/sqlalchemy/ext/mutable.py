...
which are propagated into ORM change event on owning parent objects.
...
the :class:`.Mutable` mixin to a plain Python dictionary::
    ...
    class MutableDict(Mutable, dict):
        ...
        def __setitem__(self, key, value):
            "Detect dictionary set event and emit change event."

            dict.__setitem__(self, key, value)
            self.changed()

        def __delitem__(self, key):
            "Detect dictionary del event and emit change event."
...
subclass which routes all mutation event through ``__setitem__``.  There are
...
change event to its parents via the :meth:`.MutableComposite.changed` method.
...
and to also route attribute set event via ``__setattr__`` to the
...
listeners are established which will route change event from ``Point``
...
class MutableBase(object):
    ...
    def _get_listen_keys(cls, attribute):
        ...
        :meth:`.InstanceEvents.refresh_flush` event, which pass along a list
    ...
    def _listen_on_attribute(cls, attribute, coerce, parent_cls):
        ...
        def set_(target, value, oldvalue, initiator):
            """Listen for set/replace event on the target
            data member.
...
class Mutable(MutableBase):
    ...
    def changed(self):
        """Subclasses should call this method whenever change event occur."""
...
class MutableComposite(MutableBase):
    """Mixin that defines transparent propagation of change
    event on a SQLAlchemy "composite" object to its
...
class MutableDict(Mutable, dict):
    ...
    emit change event to the underlying mapping when the contents of
    ...
    *values themselves* inside the dictionary. Therefore it is not a sufficient
    ...
    "mutable", and emit event up to their parent structure.
...
class MutableList(Mutable, list):
    ...
    *values themselves* inside the list. Therefore it is not a sufficient
    ...
    def __setitem__(self, index, value):
        """Detect list set event and emit change event."""
    ...
    def __delitem__(self, index):
        """Detect list del event and emit change event."""
