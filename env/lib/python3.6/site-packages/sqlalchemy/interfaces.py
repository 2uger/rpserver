...
.. deprecated:: 0.7
    ...
    providing a consistent interface to all event without the need for
    subclassing.
...
class PoolListener(object):
    ...
    accept event listeners for key connection lifecycle event:
    creation, pool check-out and check-in.  There are no event fired
    when a connection closes.

    For any given DB-API connection, there will be one ``connect``
    event, `n` number of ``checkout`` event, and either `n` or `n - 1`
    ``checkin`` event.  (If a ``Connection`` is detached from its
    pool via the ``detach()`` method, it won't be checked back in.)

    These are low-level event for low-level objects: raw Python
    ...
    There is no need to subclass ``PoolListener`` to handle event.
...
class ConnectionProxy(object):
    ...
    def execute(self, conn, execute, clauseelement, *multiparams, **params):
        """Intercept high level execute() event."""
    ...
    def cursor_execute(
        ...
        """Intercept low-level cursor execute() event."""
    ...
    def rollback(self, conn, rollback):
        """Intercept rollback() event."""

        return rollback()

    def commit(self, conn, commit):
        """Intercept commit() event."""
